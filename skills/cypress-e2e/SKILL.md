...
1. **Page Reflection & API Interface Validation**: Validate pages reflect API state changes correctly (loading, empty, success, errors). Mock responses using `cy.intercept` and external JSON fixtures (stored in `cypress/fixtures/`).
...
1. **Gather API/UI Specs**: Identify the page component, API endpoints, mock payloads (JSON fixtures), and target states.
...
3. **Implement Cypress Tests**:
   ...
   - Setup appropriate `cy.intercept` calls referencing JSON fixtures to handle mock inputs.
